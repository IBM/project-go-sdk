...
var _ = Describe(`ProjectV1 Examples Tests`, func() {
	...
	Describe(`ProjectV1 request examples`, func() {
		...
		It(`CreatePrevalidate request example`, func() {
			fmt.Println("\nCreatePrevalidate() result:")
			// begin-create_prevalidate
			...
			createPrevalidateOptions := projectService.NewCreatePrevalidateOptions(
				"testString",
				"testString",
				projectConfigDefinitionPrototypeModel,
			)
			...
			// end-create_prevalidate

			Expect(err).To(BeNil())
			Expect(response.StatusCode).To(Equal(202))
			Expect(result).ToNot(BeNil())
		})
		It(`GetPrevalidate request example`, func() {
			fmt.Println("\nGetPrevalidate() result:")
			// begin-get_prevalidate

			getPrevalidateOptions := projectService.NewGetPrevalidateOptions(
				"testString",
				"testString",
				"testString",
			)
			...
			b, _ := json.MarshalIndent(prevalidateGetResponse, "", "  ")
			fmt.Println(string(b))
		...
		It(`ListConfigVersions request example`, func() {
			...
			b, _ := json.MarshalIndent(projectConfigVersionCollection, "", "  ")
			fmt.Println(string(b))
		...
		It(`DeleteConfigVersionV2 request example`, func() {
			fmt.Println("\nDeleteConfigVersionV2() result:")
			// begin-delete_config_version_v2

			deleteConfigVersionV2Options := projectService.NewDeleteConfigVersionV2Options(
				"testString",
				"testString",
				int64(0),
			)
